...
describe('basic > ', () => {
  ...
  describe('useCart > ', () => {
    ...
    test('제품 수량을 업데이트해야 합니다', () => {
      ...
      act(() => result.current.addToCart(testProduct));
      act(() => result.current.updateQuantity(testProduct.id, 5));
    ...
    test('합계를 정확하게 계산해야 합니다', () => {
      ...
      act(() => result.current.addToCart(testProduct));
      act(() => result.current.updateQuantity(testProduct.id, 2));
      act(() => result.current.applyCoupon(testCoupon));
